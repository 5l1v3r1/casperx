...
class XSSerOptions(optparse.OptionParser):
    def __init__(self, *args):
        optparse.OptionParser.__init__(self,
                           description='CasperCross is a Lightweight XSS Exploitation Framework for Windows based on XSSer',
                           prog='XSSer.py',
			   version='\n XSSer v1.7b: "for windows" - 2016 - (GPLv3.0) -> by psy | Sillhouette\n',
                           usage= '\n\ncasperx [OPTIONS] [--all <url> |-u <url> |-i <file> |-d <dork> (options)|-l ] [-g <get> |-p <post> |-c <crawl> (options)]\n[Request(s)] [Checker(s)] [Vector(s)] [Anti-antiXSS/IDS] [Bypasser(s)] [Technique(s)] [Final Injection(s)] [Reporting] {Miscellaneous}')
        ...
        self.add_option("--gtk", action="store_true", dest="xsser_gtk", help="GUI Version will be available soon")
    ...
    def get_options(self, user_args=None):
        ...
        if (not options.url and not options.readfile and not options.dork and not options.dork_file and not options.imx and not options.flash and not options.update and not options.xsser_gtk and not options.wizard and not options.xst and not options.target):
            ...
            print "From: donationcasper@gmail.com"
            coloramainit()
            print (Fore.RED + '''
$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
$$$$$$$$$$$*****$$$$$$$$$$$$$$
...
$$$$$$$$$$$$$$$$$$$$$Indonesia''')
            print ""
            print '='*75
            print "Total exploits:", self.total_vectors + " = XSS: " + str(self.vectors_fuzz) + " + DCP: " + str(self.vectors_dcp) + " + DOM: " + str(self.vectors_dom) + " + HTTPsr: " + str(self.vectors_httpsr)
            print '='*75
